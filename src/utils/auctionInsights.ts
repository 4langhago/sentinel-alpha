...
export function buildInsights(
  ...
  if (item.discount_rate !== null && item.fail_count > 0) {
    out.push({
      ...
      // "유찰 1회당 10% 저감"은 흔히 인용되지만 우리 데이터와 맞지 않는다:
      // 압류재산 유찰 0회 50,834건 중 최저가가 감정가의 95% 이상인 것은 10.5%뿐이다.
      // 1회차 최저가가 감정가와 같다는 전제가 성립하지 않으므로, 저감 규칙을
      // 단정하지 않고 관측된 값만 말한다.
      basis:
        `${item.fail_count}회 유찰을 거쳐 최저입찰가가 감정가의 ${item.discount_rate}%가 됐다. ` +
        '회차별 저감 폭은 공고기관과 물건에 따라 다르므로 다음 회차 최저가는 공고문에서 확인해야 한다.',
    })
  }
  ...
  // ── 지나치게 낮은 최저가 ────────────────────────────────
  //
  // 이 자리에 한때 "유찰 N회면 통상 감정가의 0.9^N% 수준인데 실제는 더 낮다"는
  // 경고를 넣으려 했다가 버렸다. 실측으로 그 모델이 성립하지 않는다:
  // 압류재산 중 유찰 0회인 50,834건에서 최저가가 감정가의 95% 이상인 것은
  // 10.5%뿐이다. 1회차 최저가가 감정가와 같다는 전제 자체가 틀렸다.
  // 없는 규칙으로 "설명되지 않는다"고 단정하면 74%의 물건에 근거 없는
  // 경고가 붙는다 — 경고가 그만큼 흔해지면 아무도 읽지 않는다.
  //
  // 그래서 유찰 횟수로 원인을 추정하지 않고, 관측된 사실(최저가가 감정가 대비
  // 얼마나 낮은가)만 말한다. 유찰 자체를 탓하지 않으므로 점수의 유찰 축과도
  // 어긋나지 않는다. 경계 30%는 점수의 체감률 꼭대기 하한과 같은 값이다.
  if (item.discount_rate !== null && item.discount_rate < 30 && priced) {
    out.push({
      kind: 'warning',
      label: '지나치게 낮은 최저가',
      basis:
        (item.fail_count > 0
          ? `${item.fail_count}회 유찰을 거쳐 최저입찰가가 감정가의 ${item.discount_rate}%다. `
          : `유찰 없이 1회차부터 최저입찰가가 감정가의 ${item.discount_rate}%다. `) +
        '이 수준까지 낮은 물건은 지분·맹지·분묘·법정지상권처럼 시장이 기피하는 사유가 ' +
        '있거나 감정가 산정에 문제가 있는 경우가 많다. 공고문의 저가 사유와 ' +
        '등기부등본을 확인하기 전에는 싸다고 판단할 수 없다.',
      emphasis: true,
    })
  }
